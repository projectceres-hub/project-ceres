...
def register_template_sync_job(
    ...
    """
    from pantheon.serritor.jobs import sync_templates_job
...
def register_srd_index_job(
    ...
    """
    from pantheon.serritor.jobs import rebuild_srd_index_job
...
def register_session_reminder_job(
    ...
    """
    from pantheon.serritor.jobs import session_reminder_job
...
def register_cache_clean_job(
    ...
    """
    from pantheon.serritor.jobs import clean_cache_job
...
def register_snapshot_job(
    ...
    """
    Register a recurring daily snapshot job.
    
    Registers a job that creates daily snapshots of the vault by copying
    all .md files to a dated snapshot directory. Default interval is 24 hours.
    ...
    Args:
        ...
        context: Context object (should be Config or have compatible attributes):
            - current_vault: Name of the current vault
            - vaults: Dictionary mapping vault names to paths
        interval_seconds: Time interval between snapshots in seconds (default: 86400 = 24 hours)
    """
    from pantheon.serritor.jobs import daily_snapshot_job
    
    def snapshot_job() -> None:
        """Wrapper function for the daily snapshot job."""
        daily_snapshot_job(context)
    
    try:
        scheduler.register_job(
            name="daily-snapshot",
            interval_seconds=float(interval_seconds),
            callable=snapshot_job
        )
    except ValueError:
        # Job already registered, skip
        pass
...
def register_default_jobs(scheduler: Scheduler, context: Any) -> None:
    ...
    try:
        from pantheon.serritor.jobs import register_backup_job
    ...
    # Daily snapshot job - runs every 24 hours
    try:
        register_snapshot_job(scheduler, context, interval_seconds=24 * 60 * 60)  # 24 hours
    except ValueError:
        # Job already registered, skip
        pass
