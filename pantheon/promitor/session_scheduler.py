...
This module is part of the Promitor domain in the Pantheon architecture,
responsible for distribution, summaries, exports, and calendar files.
"""
...
if TYPE_CHECKING:
    ...
    from pantheon.convector.session_package import SessionEventPackage
else:
    # Avoid circular import at runtime
    Config = object
    SessionEventPackage = object
try:
...
def plan_session_interactively(
    prompt_input_func: Callable[[str], str]
) -> tuple[SessionInfo, Path, str] | None:
    """
    Run the interactive prompts to plan the next session, build the .ics file,
    and generate the user-facing message text.
    
    This is an internal helper that extracts the common interactive planning
    logic. It can be reused by both `session-schedule` and `session-discord-export`
    commands without duplicating code.
    
    Args:
        prompt_input_func: Function to get user input (takes prompt string, returns response)
        
    Returns:
        Tuple of (SessionInfo, ics_path, message_text) if successful, None if error occurred
        
    Note:
        This function creates the .ics file and exports directory as part of its work.
        The returned ics_path points to the created file.
    """
    try:
        ...
        return (info, created_path, share_message)
...
def schedule_next_session(prompt_input_func: Callable[[str], str]) -> Optional[str]:
    ...
    info, created_path, share_message = result
...
def get_timezone_string(dt: datetime) -> str:
    """
    Extract timezone identifier string from a datetime object.
    
    Attempts to get a timezone identifier (e.g., "America/New_York") from
    the datetime's tzinfo. Falls back to UTC offset or "UTC" if unavailable.
    
    Args:
        dt: Datetime object (timezone-aware or naive)
        
    Returns:
        Timezone identifier string (e.g., "America/New_York", "UTC-05:00", or "UTC")
    """
    if dt.tzinfo is None:
        # Naive datetime - try to get local timezone
        local_tz = get_local_timezone()
        if local_tz is not None:
            # Try to get timezone name from ZoneInfo
...
def build_session_event_package_from_scheduler(
    info: SessionInfo,
    ics_path: Path,
    message_text: str,
    ...
    """
    Build a SessionEventPackage from Promitor's session scheduler data.
    
    This is a convenience wrapper that converts SessionInfo and related data
    into a SessionEventPackage suitable for external systems (e.g., Discord bots).
    
    Args:
        info: SessionInfo from the scheduler
        ics_path: Path to the generated .ics calendar file
        message_text: The formatted "Next Session" message from generate_share_message
        google_calendar_url: Optional Google Calendar deep-link URL
        
    Returns:
        SessionEventPackage ready for external distribution
        
    Note:
        This function does not change existing scheduler behavior. It is a pure
        data transformation utility for future integrations.
    """
    from pantheon.convector import build_session_event_package
    ...
    timezone_str = get_timezone_string(info.start)
    
    return build_session_event_package(
        title=info.title,
...
def get_next_session_info(config: "Config") -> Optional[SessionInfo]:
    ...
    try:
        ...
        try:
            # Parse ICS datetime format: YYYYMMDDT%H%M%S
